...
@if($editable && (Gate::check('supplier.modify', $object) || Gate::check('gas.permissions', $currentgas)))
    <button class="btn btn-default pull-right async-modal" data-target-url="{{ url('/roles/supplier/' . $object->id) }}">{{ _i('Edita Ruoli') }} <span class="glyphicon glyphicon-modal-window" aria-hidden="true"></span></button>
@endif
