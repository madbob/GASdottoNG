<x-larastrap::modal classes="order-document-download-modal">
    <x-larastrap::field tlabel="generic.download">
        <a class="btn btn-light" href="{{ route('receipts.download', $receipt->id) }}">{{ __('generic.click_here') }} <i class="bi-download"></i></a>
    </x-larastrap::field>
    ...
    <x-larastrap::field tlabel="generic.forward">
        <form class="modal-form" method="GET" action="{{ route('receipts.download', $receipt->id) }}">
            ...
            <button type="submit" class="btn btn-light">{{ __('generic.forward') }}</button>
        </form>
    </x-larastrap::field>
</x-larastrap::modal>
